                      ...
                      <div onClick={() => { setSelectedMember(member); setView("person"); }} style={{ padding: "12px 16px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div style={{ flex: 1 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                            ...
                            {overdueCount > 0 && <span style={{ background: "#fff0ee", color: BWL.orange, borderRadius: 20, padding: "1px 8px", fontSize: 10, fontWeight: 700 }}>{overdueCount} OVERDUE</span>}
                          </div>
                ...
                {selectedMember && (() => {
                  ...
                  return (
                    <div>
                            ...
                            <div key={i} onClick={() => toggleCheck(selectedMember, i)}
                              ...
                              <div style={{ flex: 1 }}>
                                ...
                                <div style={{ display: "flex", gap: 8, marginTop: 5 }}>
                                  <span style={{ fontSize: 10, color: pColor(t.priority), fontWeight: 700 }}>{t.priority?.toUpperCase()}</span>
                                  {t.due && <span style={{ fontSize: 10, color: overdue?BWL.orange:"#999" }}>· {overdue?"OVERDUE — ":""}{t.due}</span>}
                                </div>
                              </div>
                            </div>
                          );
                      ...
                      {d.blockers?.length > 0 && <div style={{ background: "#fff8f0", border: `1px solid ${BWL.orange}33`, borderRadius: 10, padding: "12px 14px", marginTop: 12 }}><div style={{ fontSize: 10, color: BWL.orange, fontWeight: 700, marginBottom: 6 }}>BLOCKERS</div>{d.blockers.map((b,i)=><div key={i} style={{fontSize:12,color:BWL.darkGray}}>{b}</div>)}</div>}
                    </div>
                  );
                })()}
              </div>
...
function Dashboard() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {tasks ? (
        <Card><CardHeader label="TEAM TASK PROGRESS" /><div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>{TEAM_OPS.map(member => { const p = getProgress(member); if (p === null) return null; const memberTasks = tasks?.team_tasks?.[member]?.tasks || []; const overdue = memberTasks.filter((t, i) => !checked[`${member}-${i}`] && (() => { if (!t.due_day) return false; const days = ["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"]; return days.indexOf(t.due_day) < new Date().getDay(); })()).length; return (<div key={member} style={{ display: "flex", alignItems: "center", gap: 12 }}><div style={{ width: 140, fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{member.split(" ")[0]}</div><div style={{ flex: 1, background: BWL.lightGray, height: 6, overflow: "hidden" }}><div style={{ height: "100%", width: `${p}%`, background: p === 100 ? "#10b981" : BWL.orange, transition: "width 0.3s" }} /></div><div style={{ width: 36, fontSize: 12, fontWeight: 900, color: p === 100 ? "#10b981" : BWL.orange, textAlign: "right" }}>{p}%</div>{overdue > 0 && <div style={{ fontSize: 10, color: BWL.orange, fontWeight: 700, whiteSpace: "nowrap" }}>{overdue} overdue</div>}</div>); })}</div></Card>
      ) : (
        <Card style={{ padding: 24, textAlign: "center" }}><div style={{ fontSize: 11, color: BWL.gray, fontFamily: BWL.mono, marginBottom: 12 }}>NO TASKS GENERATED YET</div><div style={{ fontSize: 13, color: BWL.black }}>Go to OPS PULSE to generate this week's tasks.</div></Card>
      )}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
        <Card><CardHeader label="RFP PIPELINE" /><div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 8 }}>{rfpTracker.length === 0 ? <div style={{ fontSize: 12, color: BWL.gray, fontFamily: BWL.mono }}>No proposals saved yet.</div> : [["Total", rfpTracker.length, BWL.black], ["Submitted", rfpTracker.filter(t=>t.status==="submitted").length, "#f59e0b"], ["Won", won.length, "#10b981"], ["Lost", rfpTracker.filter(t=>t.status==="lost").length, "#ef4444"]].map(([l, v, c]) => (<div key={l} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: `1px solid ${BWL.lightGray}` }}><div style={{ fontSize: 11, color: BWL.gray, fontFamily: BWL.mono }}>{l}</div><div style={{ fontSize: 16, fontWeight: 900, color: c }}>{v}</div></div>))}{totalRev > 0 && <div style={{ marginTop: 8, background: "#10b98111", padding: "10px 12px" }}><div style={{ fontSize: 9, color: "#10b981", fontWeight: 900, letterSpacing: 2, marginBottom: 4 }}>REVENUE WON</div><div style={{ fontSize: 20, fontWeight: 900, color: "#10b981" }}>${totalRev.toLocaleString()}</div></div>}</div></Card>
        <Card><CardHeader label="INFLUENCER TRACKER" /><div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 8 }}>{influencers.length === 0 ? <div style={{ fontSize: 12, color: BWL.gray, fontFamily: BWL.mono }}>No influencers tracked yet.</div> : [["Total", influencers.length, BWL.black], ["Active", activeInf, "#10b981"], ["Negotiating", negoInf, "#f59e0b"], ["Paid", influencers.filter(i=>i.status==="paid").length, "#6c63ff"], ["Completed", influencers.filter(i=>i.status==="completed").length, BWL.gray]].map(([l, v, c]) => (<div key={l} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: `1px solid ${BWL.lightGray}` }}><div style={{ fontSize: 11, color: BWL.gray, fontFamily: BWL.mono }}>{l}</div><div style={{ fontSize: 16, fontWeight: 900, color: c }}>{v}</div></div>))}</div></Card>
      </div>
    </div>
  );
}
...
function RFPEngine() {
  ...
  return (
    <div>
      ...
      {view==="search" && <div>
        <Card style={{overflow:"hidden",marginBottom:14}}><CardHeader label="SEARCH RFPs" /><div style={{padding:16,display:"flex",gap:10}}><input value={keywords} onChange={e=>setKeywords(e.target.value)} onKeyDown={e=>e.key==="Enter"&&keywords.trim()&&search()} placeholder="e.g. marketing services, digital advertising..." style={{flex:1,background:BWL.bg,border:`1px solid ${BWL.lightGray}`,borderRadius:8,color:BWL.black,fontSize:13,padding:"10px 14px",outline:"none",fontFamily:"inherit"}} /><button onClick={search} disabled={!keywords.trim()||loading.search} style={{background:keywords.trim()?BWL.black:"#ccc",color:BWL.white,border:"none",borderRadius:8,padding:"10px 20px",fontSize:13,fontWeight:700,cursor:keywords.trim()?"pointer":"not-allowed"}}>{loading.search?"SEARCHING...":"SEARCH"}</button></div></Card>
        <Err msg={error} />
        {rfps.length>0&&!proposal&&<div style={{display:"flex",flexDirection:"column",gap:10}}>{rfps.map((rfp,i)=>{ const urg=urgencyTag(rfp.deadline), isExp=expandedScore===rfp.id; return (<Card key={i} style={{padding:18,border:`1px solid ${selected?.id===rfp.id?BWL.orange:BWL.lightGray}`}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}><div style={{flex:1}}><div style={{fontWeight:700,fontSize:14,marginBottom:4}}>{rfp.title}</div><div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}><span style={{fontSize:11,color:BWL.gray}}>{rfp.organization}</span><span style={{background:"#f0f0f0",color:BWL.darkGray,borderRadius:20,padding:"2px 8px",fontSize:10,fontWeight:700}}>{rfp.type}</span>{urg&&<span style={{background:urg.color+"18",color:urg.color,borderRadius:20,padding:"2px 8px",fontSize:10,fontWeight:700}}>{urg.label}</span>}</div></div><div style={{textAlign:"center",marginLeft:12}}><div style={{fontSize:26,fontWeight:800,color:sc(rfp.relevance_score)}}>{rfp.relevance_score}</div><button onClick={()=>setExpandedScore(isExp?null:rfp.id)} style={{fontSize:9,color:BWL.orange,fontWeight:700,background:"none",border:"none",cursor:"pointer",padding:0}}>{isExp?"HIDE":"WHY"}</button></div></div>{isExp&&rfp.score_breakdown&&(<div style={{background:BWL.bg,borderRadius:8,padding:12,marginBottom:10}}><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:6}}>SCORE BREAKDOWN</div><div style={{fontSize:12,color:BWL.darkGray,marginBottom:6}}>{rfp.score_breakdown.overall}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}><div><div style={{fontSize:9,color:"#10b981",fontWeight:900,marginBottom:3}}>STRENGTHS</div>{rfp.score_breakdown.strengths?.map((s,j)=><div key={j} style={{fontSize:11,marginBottom:2}}>· {s}</div>)}</div><div><div style={{fontSize:9,color:"#ef4444",fontWeight:900,marginBottom:3}}>GAPS</div>{rfp.score_breakdown.gaps?.map((g,j)=><div key={j} style={{fontSize:11,marginBottom:2}}>· {g}</div>)}</div></div></div>)}<p style={{margin:"0 0 8px",fontSize:12,color:BWL.darkGray,lineHeight:1.6}}>{rfp.description}</p>{rfp.services_needed?.length>0&&<div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:8}}>{rfp.services_needed.map((s,j)=><span key={j} style={{background:BWL.orange+"18",color:BWL.orange,borderRadius:20,padding:"2px 8px",fontSize:10,fontWeight:700}}>{s}</span>)}</div>}<div style={{fontSize:12,color:"#10b981",marginBottom:10}}>{rfp.why_bwl_can_win}</div><button onClick={()=>genProposal(rfp)} style={{width:"100%",padding:"9px 0",borderRadius:8,background:loading.proposal&&selected?.id===rfp.id?"#ccc":BWL.black,color:BWL.white,border:"none",fontSize:12,fontWeight:700,cursor:"pointer"}}>{loading.proposal&&selected?.id===rfp.id?"GENERATING...":"GENERATE PROPOSAL"}</button></Card>); })}</div>}
        {proposal&&selected&&<div style={{display:"flex",flexDirection:"column",gap:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2}}>PROPOSAL</div><div style={{fontSize:11,color:BWL.gray,marginTop:2}}>Template: <strong>{selected.type}</strong></div></div><button onClick={()=>{setProposal(null);setSelected(null);}} style={{background:BWL.white,color:BWL.gray,border:`1px solid ${BWL.lightGray}`,borderRadius:20,padding:"6px 14px",fontSize:11,cursor:"pointer"}}>BACK</button></div><div style={{background:BWL.black,borderRadius:12,padding:16}}><div style={{fontSize:9,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:4}}>SUBJECT</div><div style={{fontSize:14,fontWeight:700,color:BWL.white}}>{proposal.subject_line}</div></div><ResultBlock label="FULL PROPOSAL" content={proposal.full_proposal_text} copyable /><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><Bullets label="WHY BWL WINS" items={proposal.why_bwl} color="#10b981" /><Bullets label="RESULTS" items={proposal.relevant_results} color="#6c63ff" /></div><div style={{display:"flex",gap:10}}><button onClick={()=>navigator.clipboard.writeText(proposal.full_proposal_text)} style={{flex:1,padding:12,borderRadius:10,background:BWL.black,color:BWL.white,border:"none",fontSize:13,fontWeight:700,cursor:"pointer"}}>COPY</button><button onClick={()=>{saveToTracker(selected,proposal.full_proposal_text);setView("tracker");setProposal(null);setSelected(null);}} style={{flex:1,padding:12,borderRadius:10,background:"#10b981",color:BWL.white,border:"none",fontSize:13,fontWeight:700,cursor:"pointer"}}>SAVE TO PIPELINE</button></div></div>}
      </div>}
      {view==="tracker"&&<div><div style={{background:BWL.black,borderRadius:12,padding:18,marginBottom:16}}><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:12}}>WIN RATE DASHBOARD</div><div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10}}>{[["Total",tracker.length,BWL.orange],["Pending",tracker.filter(t=>t.status==="submitted").length,"#f59e0b"],["Won",won.length,"#10b981"],["Lost",tracker.filter(t=>t.status==="lost").length,"#ef4444"],["Win Rate",`${winRate}%`,winRate>=50?"#10b981":winRate>=30?"#f59e0b":"#ef4444"]].map(([l,v,c])=>(<div key={l} style={{textAlign:"center",background:"#ffffff0d",borderRadius:10,padding:"12px 6px"}}><div style={{fontSize:20,fontWeight:800,color:c}}>{v}</div><div style={{fontSize:9,color:"#aaa",fontWeight:700,marginTop:2}}>{l.toUpperCase()}</div></div>))}</div>{(totalRev>0||pipelineRev>0)&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:10}}>{totalRev>0&&<div style={{background:"#10b98118",borderRadius:8,padding:"10px 14px"}}><div style={{fontSize:9,color:"#10b981",fontWeight:900}}>REVENUE WON</div><div style={{fontSize:18,fontWeight:800,color:"#10b981"}}>${totalRev.toLocaleString()}</div></div>}{pipelineRev>0&&<div style={{background:"#f59e0b18",borderRadius:8,padding:"10px 14px"}}><div style={{fontSize:9,color:"#f59e0b",fontWeight:900}}>PIPELINE VALUE</div><div style={{fontSize:18,fontWeight:800,color:"#f59e0b"}}>${pipelineRev.toLocaleString()}</div></div>}</div>}</div>{tracker.length===0?<div style={{textAlign:"center",padding:"40px 20px",color:BWL.gray}}><div style={{fontSize:14,fontWeight:700}}>No proposals saved yet</div></div>:<div style={{display:"flex",flexDirection:"column",gap:10}}>{tracker.map(t=>{ const urg=urgencyTag(t.deadline); return(<Card key={t.id} style={{padding:18}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><div style={{flex:1}}><div style={{fontWeight:700,fontSize:14}}>{t.title}</div><div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center",marginTop:4}}><span style={{fontSize:11,color:BWL.gray}}>{t.organization}</span>{t.type&&<span style={{background:"#f0f0f0",color:BWL.darkGray,borderRadius:20,padding:"2px 8px",fontSize:10,fontWeight:700}}>{t.type}</span>}{urg&&<span style={{background:urg.color+"18",color:urg.color,borderRadius:20,padding:"2px 8px",fontSize:10,fontWeight:700}}>{urg.label}</span>}{t.score&&<span style={{background:sc(t.score)+"18",color:sc(t.score),borderRadius:20,padding:"2px 8px",fontSize:10,fontWeight:700}}>SCORE: {t.score}</span>}</div>{t.services?.length>0&&<div style={{display:"flex",gap:4,flexWrap:"wrap",marginTop:6}}>{t.services.map((s,j)=><span key={j} style={{background:BWL.orange+"18",color:BWL.orange,borderRadius:20,padding:"2px 7px",fontSize:9,fontWeight:700}}>{s}</span>)}</div>}</div><span style={{fontSize:12,fontWeight:700,color:ss(t.status).color,whiteSpace:"nowrap",marginLeft:10}}>{ss(t.status).label}</span></div><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8,background:BWL.bg,borderRadius:8,padding:"8px 12px"}}><span style={{fontSize:11,color:BWL.gray,fontWeight:700,whiteSpace:"nowrap"}}>EST. REVENUE</span>{editRev[t.id]?(<input autoFocus value={t.revenue||""} onChange={e=>updateField(t.id,"revenue",e.target.value)} onBlur={()=>setEditRev(p=>({...p,[t.id]:false}))} placeholder="e.g. $5,000" style={{flex:1,background:BWL.white,border:`1px solid ${BWL.lightGray}`,borderRadius:6,color:BWL.black,fontSize:12,padding:"4px 8px",outline:"none",fontFamily:"inherit"}} />):(<span onClick={()=>setEditRev(p=>({...p,[t.id]:true}))} style={{flex:1,fontSize:12,color:t.revenue?BWL.black:BWL.gray,cursor:"pointer"}}>{t.revenue||"Click to add..."}</span>)}</div><div style={{marginBottom:10}}><div style={{fontSize:9,color:BWL.gray,fontWeight:700,marginBottom:4}}>NOTES</div>{editNote[t.id]?(<textarea autoFocus value={t.notes||""} onChange={e=>updateField(t.id,"notes",e.target.value)} onBlur={()=>setEditNote(p=>({...p,[t.id]:false}))} style={{width:"100%",minHeight:60,background:BWL.bg,border:`1px solid ${BWL.orange}`,borderRadius:8,color:BWL.black,fontSize:12,padding:"8px 10px",outline:"none",fontFamily:"inherit",resize:"vertical",boxSizing:"border-box"}} />):(<div onClick={()=>setEditNote(p=>({...p,[t.id]:true}))} style={{background:BWL.bg,borderRadius:8,padding:"8px 10px",fontSize:12,color:t.notes?BWL.black:BWL.gray,cursor:"pointer",minHeight:32,lineHeight:1.5}}>{t.notes||"Click to add notes..."}</div>)}</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{t.status==="draft"&&<button onClick={()=>updateStatus(t.id,"submitted")} style={{background:BWL.bg,color:"#f59e0b",border:"1px solid #f59e0b33",borderRadius:8,padding:"6px 12px",fontSize:11,fontWeight:600,cursor:"pointer"}}>SUBMITTED</button>}{t.status==="submitted"&&<><button onClick={()=>updateStatus(t.id,"won")} style={{background:BWL.bg,color:"#10b981",border:"1px solid #10b98133",borderRadius:8,padding:"6px 12px",fontSize:11,fontWeight:600,cursor:"pointer"}}>WON</button><button onClick={()=>updateStatus(t.id,"lost")} style={{background:BWL.bg,color:"#ef4444",border:"1px solid #ef444433",borderRadius:8,padding:"6px 12px",fontSize:11,fontWeight:600,cursor:"pointer"}}>LOST</button></>}<button onClick={()=>navigator.clipboard.writeText(t.proposal)} style={{background:BWL.bg,color:BWL.orange,border:`1px solid ${BWL.lightGray}`,borderRadius:8,padding:"6px 12px",fontSize:11,fontWeight:600,cursor:"pointer"}}>COPY</button><button onClick={()=>del(t.id)} style={{marginLeft:"auto",background:BWL.bg,color:BWL.gray,border:`1px solid ${BWL.lightGray}`,borderRadius:8,padding:"6px 12px",fontSize:11,cursor:"pointer"}}>DELETE</button></div></Card>); })}</div>}</div>}
    </div>
  );
}
...
function WeeklyReport() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><Textarea label="YOUR UPDATES THIS WEEK" value={updates} onChange={setUpdates} placeholder={`Type your updates for ${weekLabel()}...`} /><Textarea label="SLACK / NOTES (OPTIONAL)" value={slack} onChange={setSlack} placeholder="Paste relevant Slack messages..." minHeight={80} /><Btn onClick={gen} disabled={!updates.trim()} loading={loading} label={`GENERATE WEEKLY REPORT — ${weekLabel()}`} /><Err msg={error} />{result && <div id="weekly-report-pdf"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,gap:10}}><div style={{background:BWL.black,borderRadius:0,padding:18,flex:1}}><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:8}}>TL;DR FOR DAVID</div><p style={{margin:0,color:BWL.white,fontSize:14,lineHeight:1.7}}>{result.executive_summary}</p></div><ExportBtn title={`Weekly Report — ${weekLabel()}`} contentId="weekly-report-pdf" /></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><Bullets label="WINS" items={result.wins} color="#10b981" /><Bullets label="BLOCKERS" items={result.blockers?.length?result.blockers:["None"]} color="#ef4444" /></div><Bullets label="NEXT WEEK" items={result.next_week} color="#6c63ff" /><Bullets label="DAVID NEEDS TO KNOW" items={result.david_needs_to_know?.length?result.david_needs_to_know:["Nothing urgent"]} color={BWL.orange} /><ResultBlock label="FULL REPORT" content={result.full_report} copyable /></div>}</div>);
}
...
function ExecComms() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>{TYPES.map(t=><button key={t.key} onClick={()=>{setType(t.key);setResult(null);}} style={{padding:"11px 14px",borderRadius:10,fontSize:12,fontWeight:700,background:type===t.key?BWL.black:BWL.white,color:type===t.key?BWL.white:BWL.gray,border:type===t.key?`2px solid ${BWL.black}`:`1px solid ${BWL.lightGray}`,cursor:"pointer",textAlign:"left"}}>{t.label}</button>)}</div><div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}><span style={{fontSize:11,color:BWL.gray,fontWeight:700}}>TONE</span>{["professional","friendly","direct","urgent"].map(t=><button key={t} onClick={()=>setTone(t)} style={{padding:"5px 14px",borderRadius:20,fontSize:11,fontWeight:700,background:tone===t?BWL.orange:BWL.white,color:tone===t?BWL.white:BWL.gray,border:tone===t?"none":`1px solid ${BWL.lightGray}`,cursor:"pointer",textTransform:"uppercase"}}>{t}</button>)}</div><Textarea label="CONTEXT" value={context} onChange={setContext} placeholder="What do you need to communicate?" /><Btn onClick={gen} disabled={!context.trim()} loading={loading} label="DRAFT COMMS" /><Err msg={error} />{result && <>{result.subject&&<div style={{background:BWL.black,borderRadius:12,padding:"14px 18px"}}><div style={{fontSize:9,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:4}}>SUBJECT</div><div style={{fontSize:14,fontWeight:700,color:BWL.white}}>{result.subject}</div></div>}<ResultBlock label="MAIN DRAFT" content={result.draft} copyable /><ResultBlock label="ALTERNATIVE" content={result.alt_version} color={BWL.gray} copyable /><Bullets label="TIPS" items={result.tips} color={BWL.orange} /></>}</div>);
}
...
function DailyBriefing() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><Textarea label="PASTE UPDATES, REPORTS, SLACK MESSAGES" value={input} onChange={setInput} placeholder="Paste anything that needs to be briefed on today..." /><Btn onClick={gen} disabled={!input.trim()} loading={loading} label="GENERATE DAILY BRIEFING" /><Err msg={error} />{result && <><div style={{background:BWL.black,borderRadius:12,padding:18}}><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:8}}>TL;DR</div><p style={{margin:0,color:BWL.white,fontSize:14,lineHeight:1.7}}>{result.summary}</p></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><Bullets label="URGENT" items={result.urgent_items} color="#ef4444" /><Bullets label="DECISIONS NEEDED" items={result.decisions_needed} color="#6c63ff" /><Bullets label="FYI" items={result.fyi_items} color="#f59e0b" /></div><ResultBlock label="FULL BRIEFING" content={result.full_briefing} copyable /></>}</div>);
}
...
function TeamPerformance() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><Textarea label="PASTE TEAM UPDATES, REPORTS, OR NOTES" value={input} onChange={setInput} placeholder="Paste any team updates, SOD reports, task completions..." /><Btn onClick={gen} disabled={!input.trim()} loading={loading} label="ANALYZE TEAM PERFORMANCE" /><Err msg={error} />{result && <><div style={{background:BWL.black,borderRadius:12,padding:18}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2}}>TEAM HEALTH</div><span style={{background:hColor(result.overall_health)+"22",color:hColor(result.overall_health),borderRadius:20,padding:"4px 14px",fontSize:12,fontWeight:900,textTransform:"uppercase"}}>{result.overall_health==="green"?"HEALTHY":result.overall_health==="yellow"?"WATCH":"CRITICAL"}</span></div><p style={{margin:0,color:BWL.white,fontSize:13,lineHeight:1.7}}>{result.summary}</p></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><Bullets label="TOP PERFORMERS" items={result.top_performers} color="#10b981" /><Bullets label="NEEDS ATTENTION" items={result.needs_attention} color="#ef4444" /></div><Bullets label="TEAM INSIGHTS" items={result.team_insights} color="#6c63ff" /><Bullets label="RECOMMENDED ACTIONS" items={result.recommended_actions} color="#f59e0b" /><Card style={{padding:16,border:`1px solid ${BWL.orange}33`,background:"#fff8f0"}}><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:8}}>DAVID'S FOCUS THIS WEEK</div><p style={{margin:0,fontSize:13,color:BWL.black,lineHeight:1.6}}>{result.david_focus}</p></Card></>}</div>);
}
...
function StrategicDecision() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><Textarea label="SITUATION / DECISION" value={situation} onChange={setSituation} placeholder="Describe the strategic decision or situation..." /><Textarea label="OPTIONS BEING CONSIDERED (OPTIONAL)" value={options} onChange={setOptions} placeholder="List the options..." minHeight={80} /><Btn onClick={gen} disabled={!situation.trim()} loading={loading} label="ANALYZE DECISION" /><Err msg={error} />{result && <><div style={{background:BWL.black,borderRadius:12,padding:18}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2}}>RECOMMENDATION</div>{result.confidence&&<span style={{background:confColor(result.confidence)+"22",color:confColor(result.confidence),borderRadius:20,padding:"3px 12px",fontSize:11,fontWeight:700,textTransform:"uppercase"}}>{result.confidence} CONFIDENCE</span>}</div><p style={{margin:0,color:BWL.white,fontSize:14,lineHeight:1.7}}>{result.recommendation}</p></div>{result.pros_cons?.map((o,i)=>(<Card key={i} style={{padding:16}}><div style={{fontWeight:900,fontSize:13,marginBottom:10}}>{o.option}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><Bullets label="PROS" items={o.pros} color="#10b981" /><Bullets label="CONS" items={o.cons} color="#ef4444" /></div></Card>))}<Bullets label="NEXT STEPS" items={result.next_steps} color="#6c63ff" /><ResultBlock label="DECISION LOG" content={result.decision_log} copyable /></>}</div>);
}
...
function SequenceBuilder() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><Textarea label="TARGET AUDIENCE / ICP" value={icp} onChange={setIcp} placeholder="Who are you targeting?" minHeight={80} /><Textarea label="CAMPAIGN GOAL" value={goal} onChange={setGoal} placeholder="e.g. Book discovery call..." minHeight={70} /><Btn onClick={gen} disabled={!icp.trim()||!goal.trim()} loading={loading} label="BUILD EMAIL SEQUENCE" /><Err msg={error} />{result && <><div style={{background:BWL.black,borderRadius:12,padding:16}}><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:4}}>SEQUENCE</div><div style={{fontSize:15,fontWeight:900,color:BWL.white}}>{result.sequence_name}</div></div>{result.emails?.map((e,i)=>(<Card key={i} style={{padding:18}}><div style={{display:"flex",gap:10,alignItems:"center",marginBottom:10}}><span style={{background:BWL.black,color:BWL.white,borderRadius:20,padding:"3px 12px",fontSize:11,fontWeight:900}}>EMAIL {e.step}</span><span style={{fontSize:11,color:BWL.gray}}>{e.send_day}</span><span style={{fontSize:11,color:"#6c63ff",marginLeft:"auto"}}>{e.goal}</span></div><div style={{fontSize:12,fontWeight:700,marginBottom:8}}>Subject: {e.subject}</div><div style={{fontSize:13,lineHeight:1.7,whiteSpace:"pre-wrap",background:BWL.bg,borderRadius:8,padding:12}}>{e.body}</div></Card>))}<Bullets label="TIPS" items={result.tips} color={BWL.orange} /></>}</div>);
}
...
function LeadResearch() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><Textarea label="COMPANY / LEAD TO RESEARCH" value={target} onChange={setTarget} placeholder="Company name, website, or any lead details..." minHeight={90} /><Btn onClick={gen} disabled={!target.trim()} loading={loading} label="RESEARCH LEAD" /><Err msg={error} />{result && <><div style={{background:BWL.black,borderRadius:12,padding:18,display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}><div style={{flex:1}}><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:8}}>OVERVIEW</div><p style={{margin:0,color:BWL.white,fontSize:13,lineHeight:1.7}}>{result.company_summary}</p></div><div style={{textAlign:"center",marginLeft:20}}><div style={{fontSize:32,fontWeight:900,color:result.estimated_fit_score>=80?"#10b981":result.estimated_fit_score>=60?"#f59e0b":"#ef4444"}}>{result.estimated_fit_score}</div><div style={{fontSize:9,color:BWL.gray,fontWeight:700}}>FIT SCORE</div></div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><Bullets label="PAIN POINTS" items={result.pain_points} color="#ef4444" /><Bullets label="TALKING POINTS" items={result.talking_points} color="#6c63ff" /></div><Card style={{padding:16}}><div style={{fontSize:10,color:"#10b981",fontWeight:900,letterSpacing:2,marginBottom:6}}>WHY BWL FITS</div><p style={{margin:"0 0 10px",fontSize:13,lineHeight:1.6}}>{result.why_bwl_fits}</p><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:6}}>ANGLE</div><p style={{margin:0,fontSize:13,lineHeight:1.6}}>{result.recommended_angle}</p></Card><ResultBlock label="RESEARCH SUMMARY" content={result.research_summary} copyable /></>}</div>);
}
...
function ColdEmailWriter() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><Textarea label="LEAD INFO" value={lead} onChange={setLead} placeholder="Company, contact, role, pain points..." minHeight={90} /><Textarea label="OFFER / ANGLE (OPTIONAL)" value={offer} onChange={setOffer} placeholder="What are you pitching?" minHeight={70} /><Btn onClick={gen} disabled={!lead.trim()} loading={loading} label="WRITE COLD EMAIL" /><Err msg={error} />{result && <><div style={{background:BWL.black,borderRadius:12,padding:16}}><div style={{fontSize:9,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:4}}>SUBJECT</div><div style={{fontSize:14,fontWeight:700,color:BWL.white,marginBottom:8}}>{result.subject_line}</div><div style={{fontSize:9,color:BWL.gray,fontWeight:700,marginBottom:4}}>ALT SUBJECT</div><div style={{fontSize:13,color:"#ccc"}}>{result.alt_subject}</div></div><ResultBlock label="COLD EMAIL" content={result.email_body} copyable /><ResultBlock label="FOLLOW-UP (DAY 3)" content={result.follow_up} color={BWL.gray} copyable /><Bullets label="TIPS" items={result.tips} color={BWL.orange} /></>}</div>);
}
...
function CallScript() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><Textarea label="LEAD / COMPANY INFO" value={lead} onChange={setLead} placeholder="Who are you calling?" minHeight={90} /><Card><CardHeader label="CALL GOAL" /><input value={goal} onChange={e=>setGoal(e.target.value)} style={{width:"100%",background:"transparent",border:"none",color:BWL.black,fontSize:13,padding:"12px 16px",outline:"none",fontFamily:"inherit",boxSizing:"border-box"}} /></Card><Btn onClick={gen} disabled={!lead.trim()} loading={loading} label="GENERATE CALL SCRIPT" /><Err msg={error} />{result && <><div style={{background:BWL.black,borderRadius:12,padding:16}}><div style={{fontSize:9,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:6}}>OPENER</div><p style={{margin:"0 0 12px",color:BWL.white,fontSize:13,lineHeight:1.7}}>{result.opener}</p><div style={{fontSize:9,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:6}}>VALUE PROP</div><p style={{margin:0,color:BWL.white,fontSize:13,lineHeight:1.7}}>{result.value_prop}</p></div><Bullets label="DISCOVERY QUESTIONS" items={result.discovery_questions} color="#6c63ff" /><Card style={{padding:16}}><div style={{fontSize:10,color:"#f59e0b",fontWeight:900,letterSpacing:2,marginBottom:10}}>OBJECTION HANDLING</div>{result.objection_handling?.map((o,i)=><div key={i} style={{marginBottom:10,paddingBottom:10,borderBottom:i<result.objection_handling.length-1?`1px solid ${BWL.lightGray}`:"none"}}><div style={{fontSize:12,fontWeight:700,color:"#ef4444",marginBottom:4}}>"{o.objection}"</div><div style={{fontSize:12,lineHeight:1.5}}>→ {o.response}</div></div>)}</Card><ResultBlock label="FULL SCRIPT" content={result.full_script} copyable /></>}</div>);
}
...
function AfterCallAutomation() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><Textarea label="CALL NOTES" value={callNotes} onChange={setCallNotes} placeholder="What happened on the call? Messy notes are fine." /><Btn onClick={gen} disabled={!callNotes.trim()} loading={loading} label="GENERATE AFTER-CALL AUTOMATIONS" /><Err msg={error} />{result && <><div style={{background:BWL.black,borderRadius:12,padding:18}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2}}>CALL SUMMARY</div>{result.outcome&&<span style={{background:outColor(result.outcome)+"22",color:outColor(result.outcome),borderRadius:20,padding:"3px 12px",fontSize:11,fontWeight:700,textTransform:"uppercase"}}>{result.outcome.replace("_"," ")}</span>}</div><p style={{margin:0,color:BWL.white,fontSize:13,lineHeight:1.7}}>{result.call_summary}</p></div><ResultBlock label="CRM NOTES" content={result.crm_notes} copyable />{result.follow_up_email&&<Card style={{padding:18}}><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:10}}>FOLLOW-UP EMAIL</div><div style={{background:BWL.black,borderRadius:8,padding:"10px 14px",marginBottom:10}}><div style={{fontSize:9,color:BWL.gray,fontWeight:700,marginBottom:3}}>SUBJECT</div><div style={{fontSize:13,fontWeight:700,color:BWL.white}}>{result.follow_up_email.subject}</div></div><div style={{fontSize:13,lineHeight:1.8,whiteSpace:"pre-wrap",background:BWL.bg,borderRadius:8,padding:14}}>{result.follow_up_email.body}</div><button onClick={()=>navigator.clipboard.writeText(result.follow_up_email.body)} style={{marginTop:10,background:BWL.black,color:BWL.white,border:"none",borderRadius:8,padding:"7px 16px",fontSize:11,fontWeight:700,cursor:"pointer"}}>COPY EMAIL</button></Card>}<Card style={{padding:16,border:`1px solid ${BWL.orange}33`,background:"#fff8f0"}}><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:6}}>NEXT ACTION</div><p style={{margin:0,fontSize:13,color:BWL.black,lineHeight:1.6}}>{result.next_action}</p></Card><ResultBlock label="SLACK UPDATE" content={result.slack_update} color="#6c63ff" copyable /></>}</div>);
}
...
function InfluencerOutreach() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><Textarea label="INFLUENCER INFO" value={influencer} onChange={setInfluencer} placeholder="Name, niche, platform, followers..." minHeight={80} /><Textarea label="CAMPAIGN / BRAND" value={campaign} onChange={setCampaign} placeholder="What brand or campaign are you pitching?" minHeight={80} /><Btn onClick={gen} disabled={!influencer.trim()||!campaign.trim()} loading={loading} label="GENERATE OUTREACH" /><Err msg={error} />{result && <>{result.subject&&<div style={{background:BWL.black,borderRadius:12,padding:"14px 18px"}}><div style={{fontSize:9,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:4}}>SUBJECT</div><div style={{fontSize:14,fontWeight:700,color:BWL.white}}>{result.subject}</div></div>}<ResultBlock label="OUTREACH" content={result.outreach_message} copyable /><ResultBlock label="FOLLOW-UP (DAY 3)" content={result.follow_up} color={BWL.gray} copyable /><ResultBlock label="COLLAB BRIEF" content={result.collaboration_brief} color="#6c63ff" copyable /><Bullets label="TIPS" items={result.tips} color={BWL.orange} /></>}</div>);
}
...
function CampaignBrief() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><Textarea label="CAMPAIGN DETAILS" value={details} onChange={setDetails} placeholder="Brand, product, goal, audience, budget..." /><Btn onClick={gen} disabled={!details.trim()} loading={loading} label="BUILD CAMPAIGN BRIEF" /><Err msg={error} />{result && <><div style={{background:BWL.black,borderRadius:12,padding:18}}><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:4}}>CAMPAIGN</div><div style={{fontSize:18,fontWeight:900,color:BWL.white,marginBottom:10}}>{result.campaign_name}</div><div style={{fontSize:13,color:"#ccc",lineHeight:1.7}}>{result.objective}</div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><Bullets label="DELIVERABLES" items={result.deliverables} color="#6c63ff" /><Bullets label="KPIs" items={result.kpis} color="#10b981" /><Bullets label="DOS" items={result.dos} color="#10b981" /><Bullets label="DONTS" items={result.donts} color="#ef4444" /></div><ResultBlock label="FULL BRIEF" content={result.full_brief} copyable /></>}</div>);
}
...
function ContentTracker() {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        {[["all","ALL"],...Object.entries(statuses).map(([k,v])=>[k,v.label])].map(([k,l])=><button key={k} onClick={()=>setFilter(k)} style={{padding:"6px 14px",borderRadius:20,fontSize:11,fontWeight:700,background:filter===k?(k==="all"?BWL.black:statuses[k]?.color):BWL.white,color:filter===k?BWL.white:BWL.gray,border:filter===k?"none":`1px solid ${BWL.lightGray}`,cursor:"pointer"}}>{l} ({k==="all"?posts.length:posts.filter(p=>p.status===k).length})</button>)}
...
function DesignBrief() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><Textarea label="DESIGN REQUEST" value={request} onChange={setRequest} placeholder="What needs to be designed?" /><Btn onClick={gen} disabled={!request.trim()} loading={loading} label="GENERATE DESIGN BRIEF" /><Err msg={error} />{result && <><div style={{background:BWL.black,borderRadius:12,padding:18}}><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:4}}>PROJECT</div><div style={{fontSize:16,fontWeight:900,color:BWL.white,marginBottom:8}}>{result.project_title}</div><div style={{fontSize:13,color:"#ccc",lineHeight:1.6}}>{result.objective}</div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><Bullets label="DELIVERABLES" items={result.deliverables} color="#6c63ff" /><Bullets label="MOOD" items={result.mood} color="#a855f7" /></div><Bullets label="BRAND GUIDELINES" items={result.brand_guidelines} color={BWL.orange} /><ResultBlock label="FULL DESIGN BRIEF" content={result.full_brief} copyable /></>}</div>);
}
...
function FeedbackSummary() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><Textarea label="PASTE FEEDBACK" value={feedback} onChange={setFeedback} placeholder="Paste raw feedback — messy is fine..." /><Btn onClick={gen} disabled={!feedback.trim()} loading={loading} label="SUMMARIZE FEEDBACK" /><Err msg={error} />{result && <><div style={{background:BWL.black,borderRadius:12,padding:16}}><div style={{fontSize:10,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:6}}>OVERVIEW</div><p style={{margin:0,color:BWL.white,fontSize:13,lineHeight:1.7}}>{result.summary}</p></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><Bullets label="REQUIRED CHANGES" items={result.required_changes} color="#ef4444" /><Bullets label="NICE TO HAVE" items={result.nice_to_have} color="#f59e0b" /></div><Bullets label="KEEP AS IS" items={result.keep_as_is} color="#10b981" /><ResultBlock label="MESSAGE FOR DESIGNER" content={result.designer_message} copyable /></>}</div>);
}
      ...
      {isMobile && menuOpen && (
        <div style={{ background: BWL.black, padding: 16, display: "flex", flexDirection: "column", gap: 4, position: "sticky", top: 54, zIndex: 99 }}>
          {NAV.map(n => n.children ? (
            <div key={n.key}>
              <div style={{ fontSize: 10, color: BWL.gray, fontWeight: 900, letterSpacing: 2, padding: "8px 10px 4px" }}>{n.label}</div>
              {n.children.map(c => (
                <button key={c.key} onClick={() => navigate(c.key)}
                  style={{ display: "block", width: "100%", padding: "9px 16px", background: page === c.key ? BWL.orange : "transparent", color: BWL.white, border: "none", fontSize: 12, fontWeight: 700, cursor: "pointer", borderRadius: 6, textAlign: "left", letterSpacing: 0.5 }}>
                  {c.label}
                </button>
              ))}
            </div>
            ...
            <button key={n.key} onClick={() => navigate(n.key)}
              style={{ display: "block", width: "100%", padding: "9px 16px", background: page === n.key ? BWL.orange : "transparent", color: BWL.white, border: "none", fontSize: 12, fontWeight: 700, cursor: "pointer", borderRadius: 6, textAlign: "left", letterSpacing: 0.5 }}>
              {n.label}
            </button>
          ))}
        </div>
