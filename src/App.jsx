    ...
    <div style={{minHeight:"100vh",background:T.black,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{width:"100%",maxWidth:400,textAlign:"center"}}>
        ...
        <div style={{
          ...
          <div style={{fontSize:14,fontWeight:700,color:"#fff",fontFamily:T.font,marginBottom:6,letterSpacing:1}}>
            RESTRICTED ACCESS
          </div>
          <div style={{fontSize:12,color:"#555",fontFamily:T.mono,marginBottom:24}}>
          ...
          <input
            ...
            style={{
              width:"100%",
              background:"#1a1a1a",
              border:`1.5px solid ${error ? T.red : "#333"}`,
              borderRadius:8,
              color:"#fff",
              fontSize:15,
              padding:"12px 16px",
          ...
          {error && (
            <div style={{fontSize:12,color:T.red,fontFamily:T.mono,marginBottom:12,letterSpacing:1}}>
              ✗ INCORRECT PASSWORD
            </div>
          )}
...
const Badge = ({label,color=T.orange,bg}) => (
  <span style={{display:"inline-flex",alignItems:"center",background:bg||color+"18",color,borderRadius:4,padding:"2px 8px",fontSize:10,fontWeight:700,letterSpacing:0.8,fontFamily:T.mono,textTransform:"uppercase"}}>{label}</span>
);

const Pill = ({label,color=T.orange,active,onClick}) => (
  <button onClick={onClick} style={{padding:"5px 14px",borderRadius:20,fontSize:11,fontWeight:600,letterSpacing:0.5,background:active?color:T.surface,color:active?"#fff":T.gray,border:active?"none":`1px solid ${T.border}`,cursor:"pointer",transition:"all 0.15s",fontFamily:T.body}}>{label}</button>
);
...
const Card = ({children,style={},hover=false}) => {
  ...
  return (
    <div onMouseEnter={()=>hover&&setIsHov(true)} onMouseLeave={()=>hover&&setIsHov(false)}
      style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:12,boxShadow:isHov?"0 8px 32px rgba(0,0,0,0.10)":"0 1px 4px rgba(0,0,0,0.05)",transition:"box-shadow 0.2s",overflow:"hidden",...style}}>
      {children}
    </div>
  );
};
...
const Input = ({value,onChange,placeholder,type="text",style={},...props}) => (
  <input type={type} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder}
    style={{width:"100%",background:T.bg,border:`1.5px solid ${T.border}`,borderRadius:8,color:T.black,fontSize:13,padding:"10px 14px",outline:"none",fontFamily:T.body,transition:"border-color 0.15s",...style}}
...
const Btn = ({onClick,disabled,loading,label,color,icon,variant="primary"}) => {
  ...
  const bg = disabled?T.border:variant==="ghost"?(hov?T.bg:"transparent"):(hov?T.orangeHov:(color||T.black));
  const col = disabled?T.gray:variant==="ghost"?T.gray:"#fff";
  return (
    <button onClick={onClick} disabled={disabled||loading}
      onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{width:"100%",padding:"12px 20px",borderRadius:8,background:bg,color:col,border:variant==="ghost"?`1px solid ${T.border}`:"none",fontSize:13,fontWeight:700,cursor:disabled?"not-allowed":"pointer",letterSpacing:1,display:"flex",alignItems:"center",justifyContent:"center",gap:8,transition:"all 0.15s",fontFamily:T.font}}>
...
const CopyBtn = ({text}) => {
  ...
  return (
    <button onClick={()=>{navigator.clipboard.writeText(text);setCopied(true);setTimeout(()=>setCopied(false),2000);}}
      style={{background:copied?T.green:T.black,color:"#fff",border:"none",borderRadius:6,padding:"5px 14px",fontSize:11,fontWeight:700,cursor:"pointer",transition:"background 0.2s",fontFamily:T.mono,letterSpacing:0.5}}>
      {copied?"✓ COPIED":"COPY"}
    </button>
  );
};
...
const Err = ({msg}) => msg?(
  <div style={{background:T.orangeSoft,border:`1px solid ${T.orange}44`,borderRadius:8,padding:"12px 16px",color:T.orange,fontSize:13,display:"flex",gap:8,alignItems:"flex-start"}}>
  ...
  return (
    <Card style={{padding:16}}>
      ...
      <div style={{marginTop:10,display:"flex",flexDirection:"column",gap:6}}>
        {items.map((w,i)=><div key={i} style={{fontSize:13,color:T.darkGray,lineHeight:1.6,paddingLeft:12,borderLeft:`2px solid ${color}44`}}>{w}</div>)}
      </div>
    </Card>
  );
};
...
const ProgressBar = ({value,color,height=6}) => (
  <div style={{background:T.border,borderRadius:99,height,overflow:"hidden"}}>
    <div style={{height:"100%",width:`${value}%`,background:value===100?T.green:(color||T.orange),borderRadius:99,transition:"width 0.4s ease"}} />
  </div>
);
...
const Avatar = ({name,size=32}) => {
  ...
  return (
    <div style={{width:size,height:size,borderRadius:"50%",background:`hsl(${hue},60%,88%)`,color:`hsl(${hue},50%,35%)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:size*0.36,fontWeight:700,flexShrink:0,fontFamily:T.font}}>{initials}</div>
  );
};
...
const isSectionHeader = (line) => {
  ...
  const twoDigits = line[0] >= "0" && line[0] <= "9" && line[1] >= "0" && line[1] <= "9";
  return twoDigits && line.indexOf("//") !== -1;
};
const parseSectionHeader = (line) => {
  const slashIdx = line.indexOf("//");
  return { num: line.slice(0, 2), title: line.slice(slashIdx + 2).trim() };
};
...
const parseProposalText = (text) => {
  ...
  while (i < lines.length) {
    ...
    if (isSectionHeader(line)) {
      ...
      blocks.push({ type: "section", num: parsed.num, title: parsed.title });
      i++; continue;
    }
    if (isKeyInsight(line)) {
      const insightLines = [];
      i++;
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {proposal.requirements_checklist.map((item,i)=>(
                <div key={i} style={{display:"flex",gap:14,padding:"11px 16px",background:item.addressed?"#F0FDF4":"#FFF5F5",borderLeft:`3px solid ${item.addressed?T.green:T.red}`}}>
                  ...
                  <div>
                    <div style={{fontSize:12,fontWeight:700,color:T.black,fontFamily:T.font}}>{item.requirement}</div>
                    {item.how&&<div style={{fontSize:11,color:T.gray,marginTop:3,fontFamily:T.mono,lineHeight:1.5}}>{item.how}</div>}
                  </div>
                </div>
              ))}
            </div>
...
const DEFAULT_SLACK_IDS = {
  "David Perlov":"U08BQH5JJDD","Cyril Butanas":"U09HHPVSSUQ","Caleb Bentil":"U0AE1T4N7A8",
  "Darlene Mae Malolos":"U0A8GV25V0A","Suki Santos":"U093GFVM7D1",
  "Kristine Miel Zulaybar":"U093GFXPK3M","Kristine Mirabueno":"U09QJGY27JP",
};
...
function OpsPulse({slackIds}) {
  ...
  const sendDM = async (member,extraContext) => {
    ...
    try {
      ...
      setShowDmContext(p=>({...p,[member]:false}));
      setDmContext(p=>({...p,[member]:""}));
    } catch {setSlackStatus(p=>({...p,[member]:"FAILED"}));}
    setTimeout(()=>setSlackStatus(p=>({...p,[member]:null})),3000);
  };

  const addTask = async (member) => {
    const newTask={task:"New task",priority:"medium",due:"EOW",due_day:"Friday",type:"action"};
    const updated={...result,team_tasks:{...result.team_tasks,[member]:{...result.team_tasks[member],tasks:[...(result.team_tasks[member]?.tasks||[]),newTask]}}};
    setResult(updated);
    await storage.set("ops-pulse-current",JSON.stringify(updated));
  };

  const deleteTask = async (member,idx) => {
    const tasks=result.team_tasks[member].tasks.filter((_,i)=>i!==idx);
    const updated={...result,team_tasks:{...result.team_tasks,[member]:{...result.team_tasks[member],tasks}}};
    setResult(updated);
    const newChecked={...checked};
    delete newChecked[`${member}-${idx}`];
    setChecked(newChecked);
    await storage.set("ops-pulse-current",JSON.stringify(updated));
    await storage.set("ops-pulse-checked",JSON.stringify(newChecked));
  };

  const saveTaskEdit = async (member,idx) => {
    const tasks=[...result.team_tasks[member].tasks];
  ...
  const updateTaskField = async (member,idx,field,value) => {
    const tasks=[...result.team_tasks[member].tasks];
      ...
      {(showInput||!result)&&(
        <Card>
          <div style={{display:"flex",borderBottom:`1px solid ${T.border}`}}>
            {INPUT_TYPES.map(t=>(
              <button key={t.key} onClick={()=>setActiveTab(t.key)} style={{flex:1,padding:"12px 8px",fontSize:12,fontWeight:600,background:activeTab===t.key?T.orangeSoft:"transparent",color:activeTab===t.key?T.orange:T.gray,border:"none",borderBottom:activeTab===t.key?`2px solid ${T.orange}`:"2px solid transparent",cursor:"pointer",transition:"all 0.15s",fontFamily:T.body}}>
                  ...
                  <Card style={{padding:20}}>
                    ...
                    <div style={{marginTop:14,background:T.bg,borderRadius:10,padding:12}}>
                      {showDmContext[selectedMember]?(
                        <div style={{display:"flex",flexDirection:"column",gap:8}}>
                          <div style={{fontSize:11,color:T.gray,fontWeight:600}}>ADD NOTE TO SLACK DM (optional)</div>
                          <textarea value={dmContext[selectedMember]||""} onChange={e=>setDmContext(p=>({...p,[selectedMember]:e.target.value}))}
                            ...
                            style={{width:"100%",minHeight:70,background:T.surface,border:`1.5px solid ${T.border}`,borderRadius:8,color:T.black,fontSize:13,padding:"10px 12px",outline:"none",fontFamily:T.body,resize:"vertical"}} />
                          <div style={{display:"flex",gap:8}}>
                            <button onClick={()=>sendDM(selectedMember,dmContext[selectedMember])}
                              style={{flex:1,padding:"9px 0",borderRadius:8,background:T.black,color:"#fff",border:"none",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:T.font}}>
                              {slackStatus[selectedMember]||"SEND DM"}
                            </button>
                            <button onClick={()=>setShowDmContext(p=>({...p,[selectedMember]:false}))}
                              style={{padding:"9px 16px",borderRadius:8,background:T.surface,color:T.gray,border:`1px solid ${T.border}`,fontSize:12,cursor:"pointer",fontFamily:T.font}}>
                              CANCEL
                            </button>
                          </div>
                        </div>
                        ...
                        <div style={{display:"flex",gap:8}}>
                          ...
                          <button onClick={()=>addTask(selectedMember)}
                            style={{padding:"9px 16px",borderRadius:8,background:T.surface,color:T.orange,border:`1px solid ${T.orange}44`,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:T.font}}>
                            + ADD TASK
                          </button>
                        </div>
                      )}
                    ...
                    <div style={{marginTop:14,display:"flex",flexDirection:"column",gap:8}}>
                      {tasks.map((t,i)=>{
                        ...
                        return (
                          <div key={i} style={{background:isDone?"#F0FDF4":overdue?"#FFF7F5":T.bg,borderRadius:10,padding:"12px 14px",border:`1px solid ${isDone?T.green+"33":overdue?T.orange+"44":T.border}`}}>
                            <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
                              <div onClick={()=>!isEditingThis&&toggleCheck(selectedMember,i)}
                                style={{width:20,height:20,borderRadius:5,border:`2px solid ${isDone?T.green:overdue?T.orange:T.borderDark}`,background:isDone?T.green:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:2,cursor:"pointer"}}>
                                ...
                                {isEditingThis?(
                                  <div style={{display:"flex",flexDirection:"column",gap:8}}>
                                    <input autoFocus value={editingTaskText} onChange={e=>setEditingTaskText(e.target.value)}
                                      ...
                                      style={{width:"100%",background:T.surface,border:`1.5px solid ${T.orange}`,borderRadius:6,color:T.black,fontSize:13,padding:"6px 10px",outline:"none",fontFamily:T.body}} />
                                    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                                      <select value={t.priority} onChange={e=>updateTaskField(selectedMember,i,"priority",e.target.value)}
                                        style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:6,color:T.black,fontSize:11,padding:"4px 8px",outline:"none"}}>
                                      ...
                                      <button onClick={()=>saveTaskEdit(selectedMember,i)}
                                        style={{background:T.green,color:"#fff",border:"none",borderRadius:6,padding:"4px 12px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:T.font}}>SAVE</button>
                                      <button onClick={()=>setEditingTask(null)}
                                        style={{background:T.bg,color:T.gray,border:`1px solid ${T.border}`,borderRadius:6,padding:"4px 12px",fontSize:11,cursor:"pointer",fontFamily:T.font}}>CANCEL</button>
                                    </div>
                                  </div>
                                ):(
                                  <>
                              ...
                              {!isEditingThis&&(
                                <div style={{display:"flex",gap:2,flexShrink:0}}>
                                  <button onClick={()=>{setEditingTask(key);setEditingTaskText(t.task);}}
                                    style={{background:"transparent",border:"none",color:T.grayLight,fontSize:14,cursor:"pointer",padding:"2px 6px",borderRadius:4}}>✏️</button>
                                  <button onClick={()=>deleteTask(selectedMember,i)}
                                    style={{background:"transparent",border:"none",color:T.grayLight,fontSize:14,cursor:"pointer",padding:"2px 6px",borderRadius:4}}>🗑</button>
                                </div>
                              )}
                            </div>
                          </div>
                    ...
                    <Card key={i} style={{padding:18,border:`1px solid ${selected?.id===rfp.id?T.orange:T.border}`}} hover>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}>
                        ...
                        <div style={{textAlign:"center",marginLeft:14}}>
                          ...
                          <button onClick={()=>setExpandedScore(isExp?null:rfp.id)} style={{fontSize:10,color:T.orange,fontWeight:700,background:"none",border:"none",cursor:"pointer",padding:0,fontFamily:T.mono}}>{isExp?"HIDE":"WHY?"}</button>
                        </div>
                      </div>
          ...
          {proposal&&selected&&(
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <div style={{display:"flex",gap:8}}>
                  {selected.source_url&&<a href={selected.source_url} target="_blank" rel="noreferrer" style={{padding:"6px 14px",borderRadius:20,fontSize:11,fontWeight:700,background:T.orangeSoft,color:T.orange,border:`1px solid ${T.orange}33`}}>VIEW RFP ↗</a>}
              ...
              <Card style={{padding:18}}>
                ...
                {editingProposal&&(
                  <textarea value={editedProposalText} onChange={e=>setEditedProposalText(e.target.value)}
                    style={{width:"100%",minHeight:400,background:T.bg,border:`1.5px solid ${T.orange}`,borderRadius:8,color:T.black,fontSize:12,padding:14,outline:"none",fontFamily:T.mono,lineHeight:1.8,resize:"vertical"}} />
                )}
              </Card>
          ...
          <Card style={{background:T.black,padding:20}}>
            ...
            {(totalRev>0||pipelineRev>0)&&(
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:14}}>
                {totalRev>0&&<div style={{background:"#10b98118",borderRadius:8,padding:"10px 14px"}}><div style={{fontSize:9,color:T.green,fontWeight:900,fontFamily:T.mono}}>REVENUE WON</div><div style={{fontSize:20,fontWeight:800,color:T.green,fontFamily:T.font}}>${totalRev.toLocaleString()}</div></div>}
                {pipelineRev>0&&<div style={{background:"#f59e0b18",borderRadius:8,padding:"10px 14px"}}><div style={{fontSize:9,color:T.yellow,fontWeight:900,fontFamily:T.mono}}>PIPELINE VALUE</div><div style={{fontSize:20,fontWeight:800,color:T.yellow,fontFamily:T.font}}>${pipelineRev.toLocaleString()}</div></div>}
              </div>
            )}
          </Card>
...
function Settings({slackToken,setSlackToken,slackIds,setSlackIds,onChangePassword}) {
  ...
  const changePw=async()=>{
    ...
    await storage.set("app-password",newPw);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <Card>
        ...
        <div style={{padding:16,display:"flex",flexDirection:"column",gap:10}}>
          <div>
            <div style={{fontSize:10,color:T.gray,fontWeight:700,marginBottom:4,fontFamily:T.mono}}>NEW PASSWORD</div>
            <input type="password" value={newPw} onChange={e=>setNewPw(e.target.value)} placeholder="Min. 6 characters"
              style={{width:"100%",background:T.bg,border:`1.5px solid ${T.border}`,borderRadius:8,color:T.black,fontSize:13,padding:"10px 14px",outline:"none",fontFamily:T.mono}}
              onFocus={e=>e.target.style.borderColor=T.orange} onBlur={e=>e.target.style.borderColor=T.border} />
          </div>
          <div>
            <div style={{fontSize:10,color:T.gray,fontWeight:700,marginBottom:4,fontFamily:T.mono}}>CONFIRM PASSWORD</div>
          ...
          {pwMsg&&(
            <div style={{fontSize:12,color:pwMsg.type==="success"?T.green:T.red,fontFamily:T.mono,fontWeight:600}}>
              {pwMsg.type==="success"?"✓":"✗"} {pwMsg.text}
            </div>
          )}
          <button onClick={changePw} disabled={!newPw||!confirmPw}
            style={{padding:"11px 20px",borderRadius:8,background:newPw&&confirmPw?T.black:T.border,color:newPw&&confirmPw?"#fff":T.gray,border:"none",fontSize:13,fontWeight:700,cursor:newPw&&confirmPw?"pointer":"not-allowed",fontFamily:T.font,letterSpacing:1}}>
            UPDATE PASSWORD
          </button>
        </div>
      </Card>
...
function TopNav({page,navigate,isMobile,onLock}) {
  ...
  return (
    <header style={{background:T.black,height:58,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 28px",position:"sticky",top:0,zIndex:200,borderBottom:`3px solid ${T.orange}`,boxShadow:"0 2px 20px rgba(0,0,0,0.3)"}}>
            ...
            return (
              <div key={n.key} style={{position:"relative",display:"flex",alignItems:"stretch"}}
                ...
                <button onClick={()=>!n.children&&navigate(n.key)}
                  style={{height:"100%",padding:"0 18px",background:isActive?T.orange:"transparent",color:isActive?"#fff":"#aaa",border:"none",fontSize:12,fontWeight:700,cursor:n.children?"default":"pointer",letterSpacing:1,fontFamily:T.font,display:"flex",alignItems:"center",gap:5,transition:"background 0.15s, color 0.15s",whiteSpace:"nowrap"}}
                  ...
                  <div style={{position:"absolute",top:"100%",left:0,background:T.black,border:`1px solid #2a2a2a`,borderTop:`3px solid ${T.orange}`,minWidth:200,boxShadow:"0 12px 40px rgba(0,0,0,0.4)",zIndex:300,padding:"6px 0"}}
                    ...
                    {n.children.map(c=>(
                      <button key={c.key} onClick={()=>{navigate(c.key);setOpenGroup(null);}}
                        style={{display:"flex",alignItems:"center",width:"100%",padding:"10px 18px",background:page===c.key?"#ffffff12":"transparent",color:page===c.key?T.orange:"#ccc",border:"none",fontSize:12,fontWeight:600,cursor:"pointer",textAlign:"left",letterSpacing:0.5,fontFamily:T.body,borderLeft:page===c.key?`3px solid ${T.orange}`:"3px solid transparent",transition:"all 0.1s"}}
      ...
      {isMobile&&menuOpen&&(
        <div style={{position:"fixed",top:58,left:0,right:0,bottom:0,background:T.black,zIndex:199,overflowY:"auto",padding:16}}>
          ...
          <button onClick={()=>{onLock();setMenuOpen(false);}}
            style={{display:"block",width:"100%",padding:"10px 16px",background:"transparent",color:"#888",border:"none",fontSize:13,cursor:"pointer",borderRadius:6,textAlign:"left",marginBottom:2,fontFamily:T.body}}>
            🔒 Lock
          </button>
        </div>
      )}
  ...
  return (
    <main style={{maxWidth:920,margin:"0 auto",padding:"32px 24px 60px"}}>
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:20}}>
        {parent&&<><span style={{fontSize:11,color:T.grayLight,fontFamily:T.mono}}>{parent.label.toUpperCase()}</span><span style={{fontSize:11,color:T.borderDark}}>›</span></>}
        <span style={{fontSize:11,color:T.orange,fontWeight:700,fontFamily:T.mono,letterSpacing:1}}>{PAGE_ICONS[page]} {current?.label?.toUpperCase()||page.toUpperCase()}</span>
      </div>
      {children}
    </main>
  );
...
export default function App() {
  ...
  // Check if a custom password has been saved
  useEffect(()=>{
    storage.get("app-password").then(r=>{
      if(r?.value) setCurrentPassword(r.value);
    });
  ...
  // Override password check to use dynamic password
