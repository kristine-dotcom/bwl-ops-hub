  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 2 }}>
        {[["TEAM PROGRESS", tp ? `${tp.pct}%` : "—", tp ? `${tp.done}/${tp.total} tasks done` : "No tasks yet", tp?.pct === 100 ? "#10b981" : BWL.orange],["RFP WIN RATE", `${winRate}%`, `${won.length} won · ${rfpTracker.filter(t=>t.status==="submitted").length} pending`, winRate >= 50 ? "#10b981" : "#f59e0b"],["INFLUENCERS", `${influencers.length}`, `${activeInf} active · ${negoInf} under nego`, "#6c63ff"]].map(([label, value, sub, color]) => (
...
function ExecComms() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>{TYPES.map(t=><button key={t.key} onClick={()=>{setType(t.key);setResult(null);}} style={{padding:"11px 14px",borderRadius:10,fontSize:12,fontWeight:700,background:type===t.key?BWL.black:BWL.white,color:type===t.key?BWL.white:BWL.gray,border:type===t.key?`2px solid ${BWL.black}`:`1px solid ${BWL.lightGray}`,cursor:"pointer",textAlign:"left"}}>{t.label}</button>)}</div><div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}><span style={{fontSize:11,color:BWL.gray,fontWeight:700}}>TONE:</span>{["professional","friendly","direct","urgent"].map(t=><button key={t} onClick={()=>setTone(t)} style={{padding:"5px 14px",borderRadius:20,fontSize:11,fontWeight:700,background:tone===t?BWL.orange:BWL.white,color:tone===t?BWL.white:BWL.gray,border:tone===t?"none":`1px solid ${BWL.lightGray}`,cursor:"pointer",textTransform:"capitalize"}}>{t}</button>)}</div><Textarea label="CONTEXT" value={context} onChange={setContext} placeholder="What do you need to communicate?" /><Btn onClick={gen} disabled={!context.trim()} loading={loading} label="✉️ DRAFT COMMS" /><Err msg={error} />{result && <>{result.subject&&<div style={{background:BWL.black,borderRadius:12,padding:"14px 18px"}}><div style={{fontSize:9,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:4}}>SUBJECT</div><div style={{fontSize:14,fontWeight:700,color:BWL.white}}>{result.subject}</div></div>}<ResultBlock label="✅ MAIN DRAFT" content={result.draft} copyable /><ResultBlock label="🔀 ALTERNATIVE" content={result.alt_version} color={BWL.gray} copyable /><Bullets label="💡 TIPS" items={result.tips} color={BWL.orange} /></>}</div>);
}
...
function ColdEmailWriter() {
  ...
  return (<div style={{display:"flex",flexDirection:"column",gap:12}}><Textarea label="👤 LEAD INFO" value={lead} onChange={setLead} placeholder="Company, contact, role, pain points..." minHeight={90} /><Textarea label="🎯 OFFER / ANGLE (OPTIONAL)" value={offer} onChange={setOffer} placeholder="What are you pitching?" minHeight={70} /><Btn onClick={gen} disabled={!lead.trim()} loading={loading} label="❄️ WRITE COLD EMAIL" /><Err msg={error} />{result && <><div style={{background:BWL.black,borderRadius:12,padding:16}}><div style={{fontSize:9,color:BWL.orange,fontWeight:900,letterSpacing:2,marginBottom:4}}>SUBJECT</div><div style={{fontSize:14,fontWeight:700,color:BWL.white,marginBottom:8}}>{result.subject_line}</div><div style={{fontSize:9,color:BWL.gray,fontWeight:700,marginBottom:4}}>ALT SUBJECT</div><div style={{fontSize:13,color:"#ccc"}}>{result.alt_subject}</div></div><ResultBlock label="✉️ COLD EMAIL" content={result.email_body} copyable /><ResultBlock label="🔄 FOLLOW-UP (DAY 3)" content={result.follow_up} color={BWL.gray} copyable /><Bullets label="💡 TIPS" items={result.tips} color={BWL.orange} /></>}</div>);
}
